...
def test_score_all_markets_creates_opportunities(db_engine, monkeypatch):
    ...
    with get_session(db_engine) as session:
        ...
        for i in range(10):
            session.add(PriceSnapshot(market_id="FED-RATE-JUL", yes_bid=65, yes_ask=67, last_price=66,
                                     volume=5000, timestamp=datetime.now(timezone.utc) - timedelta(minutes=10 - i)))
        session.commit()
...
def test_score_updates_existing_opportunities(db_engine, monkeypatch):
    monkeypatch.setattr("src.ev.scorer.TRADE_PRICE_DERIVED_MODELS", True)
    Base.metadata.create_all(db_engine)
    with get_session(db_engine) as session:
        session.add(Market(market_id="FED-RATE-JUL", title="Will Fed raise rates in July?",
